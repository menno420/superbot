...
def do_open(repo, layout, sess, sector):
    ...
    git(repo, "add", "-A")
    git(repo, "commit", "-qm", f"open {sess}")
...
def do_close(repo, layout, sess, sector):
    ...
    git(repo, "add", "-A")
    git(repo, "commit", "-qm", f"close {sess}")
...
def main():
    ...
    trials = 8
    print(
        f"sessions/trial={n_sessions}  trials={trials}  "
    ...
    for concurrency in (2, 3, 5):
        ...
        for layout in ("status_quo", "per_sector", "per_claim"):
            ...
            print(
                f"  {layout:11s}: {tot_c:4d} conflicts / {tot_m:4d} merges "
