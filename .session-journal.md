...
- **Authority (autonomy boundary — maintainer decision Q-0009, 2026-06-07):**
  **docs / journal / orientation / folios are free to improve without asking** — that is the
  point of the self-improving ecosystem. **Executable config asks first:** hooks,
  `.claude/settings.json`, and the binding *rules* in `.claude/CLAUDE.md`
  (architecture / CI / layer rules) change how every future session + CI behave, so propose
  those for approval. (Adding a *pointer* or the ethos to CLAUDE.md is docs; adding an
  enforced rule or a hook is config.) Rule entries here are strong defaults Claude follows.
