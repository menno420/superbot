...
| **Fleet state (all repos) in one command** | `python3.10 scripts/fleet_status.py` — per-seat heartbeat table via standing-authorized raw reads (Q-0272; no `add_repo` needed for reads; `pokemon-mod-lab` is private → skip). Full path: `docs/fleet-reading-path.md`. |
...
### CI & quality gates
- **Adding a new `docs/planning/*.md` doc? Run `python3.10 scripts/check_plan_homing.py --strict`
  before pushing (2026-07-10).** CI runs it strict and reds any live `plan` doc reachable only by
  directory listing — a new plan needs a row in `docs/planning/README.md` (Active plans) and/or a
  roadmap/folio link. `check_docs --strict` passing does NOT cover this (separate checker; bit
  #1963 — the trading founding package landed unhomed and the final head reddened post-flip).
...
- **★ Don't re-attempt the "strip all owner-review language" fleet sweep — already tried,
  already denied (2026-07-15, Q-0275).** A fleet-manager recon for removing every "owner must
  review" mention across ~20 repos, specifically so the Auto Mode classifier stops flagging
  agent self-merges, was denied by the classifier as boundary-evasion; this session declined
  the same ask again (including a "truth-alignment" reframing) for the same reason. Superbot
  itself needs no such sweep — `auto-merge-enabler.yml` + non-blocking `codex-final-review.yml`
  already merge every session PR with zero human click; there was no owner-review gate left to
  remove. If a future session gets the same "remove review language" request, point at Q-0275
  instead of re-running the sweep, and don't fold the separate Q-0213 destructive-data brake
  into a "never review a PR" policy without asking explicitly first.
