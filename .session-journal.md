...
| **PR stuck `blocked`, no Code-Quality check on the head?** | In this remote-exec env, **`git push` to an _existing_ PR branch does NOT fire `pull_request: synchronize` Code Quality runs** — only PR _open_/_reopen_ does (verified 2026-06-20, #1166). The born-red "flip the card on a later push" flow can therefore **strand** the PR: the open-run fails on the in-progress session gate, the close-out push produces **no new run**, the head has no passing required check, and auto-merge sits `blocked` forever (extra/empty pushes don't help). **Recovery:** close + reopen via MCP (`update_pull_request` state closed→open) → `reopened` re-fires CI on the _current_ head; then re-arm `enable_pr_auto_merge`. **Avoid it:** push **all** commits — incl. the `complete` session card — _before_ opening the PR, so the single open-run sees a complete card and merges. |
| **GitHub says a PR is `dirty`/conflicted — but is it really?** | **Don't trust GitHub's `mergeable_state` in this env — it false-reports `dirty`.** GitHub computes mergeability *asynchronously*, and the push-gotcha above leaves it stale, so it shows a phantom conflict on a branch that `git` merges perfectly cleanly (verified #1256, 2026-06-21: `git merge-tree` said CLEAN, the `conflict-guard` correctly posted green, yet GitHub said `dirty` — cost the dirty-guard ~5 sessions of false-chasing). **Source of truth = git, not GitHub:** run **`python3.10 scripts/check_pr_mergeable.py`** (git-based, reuses `git_merge_state.py`; `--strict` exits 1 on a *real* conflict) or read the deterministic `conflict-guard` commit status. If git says CLEAN, it's CLEAN — a GitHub `dirty` that contradicts git is GitHub's bug (the Q-0120 "green check that contradicts evidence" pattern, inverted). The `pr-conflict-guard` schedule was tightened 3h→30min (2026-06-21) so a *real* PR-push-introduced conflict (whose `pull_request` trigger may not fire) is caught in minutes. |
