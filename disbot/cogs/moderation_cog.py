...
def _parse_member(guild: discord.Guild, text: str) -> discord.Member | None:
    ...
    return discord.utils.find(lambda m: m.name == text or m.display_name == text, guild.members)
...
class _WarnModal(discord.ui.Modal, title="Warn Member"):
    member_input = discord.ui.TextInput(label="User (mention, ID, or name)", max_length=100)
    reason_input = discord.ui.TextInput(
        label="Reason", style=discord.TextStyle.paragraph,
        required=False, placeholder="No reason provided", max_length=500,
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await db.log_mod_action(interaction.guild_id, "warn", member.id, interaction.user.id, reason)
        if count >= 3:
            try:
                until = discord.utils.utcnow() + timedelta(minutes=10)
                await member.timeout(until, reason="3 warnings reached.")
                await interaction.followup.send(f"⏳ {member.mention} timed out for 10 minutes (3 warnings).")
                await db.clear_warnings(member.id, interaction.guild_id)
            except discord.Forbidden:
                await interaction.followup.send("⚠️ 3 warnings reached but I lack permission to timeout.", ephemeral=True)


class _TimeoutModal(discord.ui.Modal, title="Timeout Member"):
    member_input = discord.ui.TextInput(label="User (mention, ID, or name)", max_length=100)
    duration_input = discord.ui.TextInput(label="Duration (minutes)", placeholder="e.g. 30", max_length=10)
    reason_input = discord.ui.TextInput(
        label="Reason", style=discord.TextStyle.paragraph,
        required=False, placeholder="No reason provided", max_length=500,
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.response.send_message(f"⏳ {member.mention} timed out for {duration} minute(s).")
            await db.log_mod_action(interaction.guild_id, "timeout", member.id, interaction.user.id, f"{duration}m: {reason}")
        except discord.Forbidden:
            await interaction.response.send_message("❌ No permission to timeout that user.", ephemeral=True)
...
class _KickModal(discord.ui.Modal, title="Kick Member"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await db.log_mod_action(interaction.guild_id, "kick", member.id, interaction.user.id, reason)
        except discord.Forbidden:
            await interaction.response.send_message("❌ No permission to kick that user.", ephemeral=True)
...
class _BanModal(discord.ui.Modal, title="Ban Member"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await db.log_mod_action(interaction.guild_id, "ban", member.id, interaction.user.id, reason)
        except discord.Forbidden:
            await interaction.response.send_message("❌ No permission to ban that user.", ephemeral=True)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        for entry in bans:
            if entry.user.name == self.username_input.value:
                ...
                await db.log_mod_action(interaction.guild_id, "unban", entry.user.id, interaction.user.id, "")
                return
        await interaction.followup.send(f"❌ No banned user found with name `{self.username_input.value}`.")
...
class ModerationCog(commands.Cog):
    ...
    def _can_act_on_interaction(self, interaction: discord.Interaction, member: Member) -> str | None:
    ...
    async def mod_menu(self, ctx):
        ...
        embed.add_field(name="⚠️ Warn", value="Issue a warning (auto-timeout at 3)", inline=True)
        embed.add_field(name="⏳ Timeout", value="Temporarily mute for N minutes", inline=True)
