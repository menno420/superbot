...
* A non-member context is denied.
...
async def actor_holds_capability(
    ...
    *,
    ...
    See the module docstring for the v1 policy.  ``guild`` is the guild whose
    per-guild overrides apply (typically ``actor.guild``); it may be ``None`` for
    the ``system`` / ``backfill`` bypass.
    """
    ...
    # 2. Must be a guild-member context.
    if actor is None or getattr(actor, "guild", None) is None:
        return CapabilityDecision(
            ...
            member_tier=None,
            reason=(
                "mutation requires a guild-member actor context "
                f"(actor_type={actor_type!r})"
            ),
        )

    # 3. Administrator floor, keyed on the declared capability.
    guild_owner_id = getattr(actor.guild, "owner_id", 0) or 0
