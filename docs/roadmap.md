...
> §4 — set by the seventh Q-0107 pass (the **P0 integrity spine AND P1-2 are now COMPLETE**;
> the priority is to **finish the P1 tier** — eval-matrix offline half + absence-guard Layer B →
> P1-3 invariants — with a **reserved slot for the active autonomous-loop / Hermes thread** (the
> Railway log-triage skill) and the owner's portable-substrate-kit OSS arc running in parallel.
> Every slot now carries a gate-state tag + a carried-slot escalation rule).
...
| Horizon | Items |
|---|---|
| **Now** | The **[band-#870 decade queue](planning/reconciliation-pass-2026-06-14-band870.md)** (seventh Q-0107 pass): the **safety/community band ✅ COMPLETE** (#772 automod · #774 logging · #775 welcome+counters) + backup posture ✅ #769/#862 + **native auto-merge migration ✅ #786/#787 (Q-0123)**. **The P0 integrity spine AND P1-2 are COMPLETE** ([hardening roadmap](planning/production-readiness/hardening-roadmap-2026-06-12.md) — **P0-3 ✅** (#777/#794 + arc PR 3 delegated-apply #817, Q-0098); **P0-4 ✅** (PR 1 #820 + PR 2 #825, Q-0100); **P0-2 ✅** (media retention #829, Q-0099); **P1-2 ✅** health-findings lifecycle #843, Q-0097). **Active Now = finish the P1 correctness tier: P1-1 eval-smoke matrix** (offline half buildable now; live half creds-gated; relates BUG-0009; **absence-guard Layer A ✅ #855**, Layer B next) **→ P1-3 invariants** — running in parallel with two active threads: the **autonomous-loop / Hermes thread** (now the dominant active thread — operating layer hardened #863/#865/#868/#869/#870; next slice = the **read-only Railway log-triage skill**, Railway access verified live #840) and the owner's **[portable substrate-kit](planning/portable-substrate-kit-extraction-2026-06-13.md)** OSS package (PR 1a/1b + 1b-tail DONE #802; PR 2 capability layer COMPLETE #805/#811/#812/#813; resume at the PR-2 remainder: modes + contract templates + triggers → PR 3 — **third carry, escalate if a fourth**). The product lanes stay open as owner-steered alternates: **mining** (V-16 phase 2 — gated on the owner's PNG pack; or structures §7.5) · **BTD6** (decode ⭐ item 3 demand-driven; owner live spot-check owed) · **AI** (§7 workflow families post-prod-check; BUG-0009 open in the [bug book](health/bug-book.md)) |
