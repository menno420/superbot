...
| A new DB table | `disbot/migrations/NNN_<name>.sql` (next free number) + new file in `disbot/utils/db/<name>.py` + register a `delete_for_guild` hook in `disbot/guild_lifecycle.py` if guild-keyed | `docs/architecture.md` INV-I; `docs/health/platform-consistency-ledger.md` § 3 |
...
| A new platform diagnostic | Register a provider in `services/diagnostics_service.py`; surface via `!platform <subcommand>` | `docs/subsystems/health-diagnostics.md`; `docs/health/platform-consistency-ledger.md` § 1 "Diagnostics"; `docs/smoke-test-checklist.md` |
| A new setting key | `disbot/utils/settings_keys/<subsystem>.py`; declare a `SettingSpec` in the cog or its service; write via `SettingsMutationPipeline` | `docs/subsystems/settings-bindings-provisioning.md`; `docs/setup-platform/settings-customization-roadmap.md`; `docs/building-roadmap/config-input-standard.md` |
| A new binding (Discord resource pointer) | Declare a `BindingSpec`; write via `BindingMutationPipeline`; never store IDs in `SettingSpec` | `docs/subsystems/settings-bindings-provisioning.md`; `docs/setup-platform/settings-customization-roadmap.md` § "Ownership invariants" |
...
| Autonomous review/approval loop (the self-improvement seams) | `superbot-review` (independent critique) + `superbot-dispatch` (fire a Claude Code Routine) skills · `scripts/check_phase_gate.py` (fix-phase vs. invent-phase) · merge/approve gates Q-0113/Q-0114 · workflow §12 | [`docs/operations/hermes-dispatch-bridge.md`](operations/hermes-dispatch-bridge.md) |
| Autonomous routine fleet (scheduled self-running routines + their prompts) | The 3 Claude Code Routines (dispatch · docs-reconciliation · night-caretaker), their triggers, the docs-only/runtime split, and version-controlled prompts | [`docs/operations/autonomous-routines.md`](operations/autonomous-routines.md) |
| Claude Code hooks (what fires when) / plugins posture | The 5 wired hooks in `.claude/settings.json` (`claude_session_start.sh`, `claude_pre_edit.py`, `claude_post_edit.py`, `claude_pr_subscribe_reminder.py`, `claude_stop_check.py`) + candidate new hooks + plugins pointer (Q-0096) | [`docs/operations/claude-code-hooks-and-plugins.md`](operations/claude-code-hooks-and-plugins.md) |
| MCP servers (CodeGraph, Context7) — what we run, when to use, pins, key setup | `.mcp.json` defines them; `enabledMcpjsonServers` approves; both pinned + provenance-noted. CodeGraph = code graph; Context7 = live library docs (`resolve-library-id`/`get-library-docs`) | [`docs/operations/mcp-servers.md`](operations/mcp-servers.md) |
...
| welcome | `cogs/welcome_cog.py` (+ `cogs/welcome/`) | embeds in `services/welcome_service.py` (no bespoke view) | `services/welcome_service.py` (greet/farewell + entry role via audited `role_automation`) — no DB writes of its own | reads via `services/welcome_config.py` (`welcome_*` KV settings) — **no migration** (Q-0110) |
| counters | `cogs/counters_cog.py` (+ `cogs/counters/`) | no view (channel-name renames) | `services/counter_service.py` (`sync_guild` — slow periodic loop, rate-limit-safe) — no DB writes of its own | reads via `services/counter_config.py` (`counters_*` KV settings) — **no migration** (Q-0110) |
| security | `cogs/security_cog.py` (+ `cogs/security/`) | no bespoke view (embeds in the cog) | `services/security_service.py` (`handle_member_join` — raid `RaidTracker` + account-age filter; kick routes through `moderation_service`) — no DB writes of its own | reads via `services/security_config.py` (`security_*` KV settings) — **no migration** (Q-0111 tiers 1+2) |
...
| project_moon | `cogs/project_moon_cog.py` | `views/projmoon/` | `services/projmoon_data_service.py` | n/a (committed `data/projmoon/`; read-only) |
