...
| `fishing`      | `fishing_catch_log`                            | **all writes via `services/fishing_workflow.py`** — `fish()` records the catch + credits coins (`economy_service.credit_in_txn`, reason `fishing_catch`) + awards `game_xp` in one transaction, events after commit; **reads** stay direct via `utils/db/games/fishing.py` |
