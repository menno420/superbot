...
|---|---|---|
| [round3-dispatch-runbook](round3-dispatch-runbook-2026-07-10.md) | the dispatch phase's working runbook — the FINAL fleet-manager founding package (v3), dispatch checklist §3, boot-verification log §5; Idea Engine + Product Forge packages drafted (queue §4) | [round3-launch-pack](round3-launch-pack-2026-07-10.md) |
| [gen3-deployment-standard](gen3-deployment-standard-2026-07-10.md) | the fast-path deployment standard for gen-3 Projects: pipelined-with-calibration-gate strategy (simulation-backed — `tools/sim/gen3_deployment_sim.py`: 113m vs sequential 296m, 5× less error exposure than big-bang), plus the repo/env/instructions/brief/routine standards (Q-0260) | [round3-dispatch-runbook](round3-dispatch-runbook-2026-07-10.md) |
| [round3-founding-package-substrate-kit](round3-founding-package-substrate-kit-2026-07-10.md) | founding package for the substrate-kit core seat (Q-0261 seat 2, the distribution seat): write-all-repos scoped to kit distribution only, routine cutover (old hourly → core 2-hourly), §6 centralization queue | Q-0261 · EAP program review §6 |
| [round3-founding-package-builder](round3-founding-package-builder-2026-07-10.md) | founding package for the Builder core seat (Q-0261 seat 3, superbot-next): rebuild at overnight pace, ORDER-008 routine, standing @codex review (Q-0259 r.3), parity/ORACLE discipline, ender catch-up | launch-readiness Seat 3 (fleet-manager `docs/launch-readiness-2026-07-10.md`) |
| [round3-founding-package-idea-engine](round3-founding-package-idea-engine-2026-07-10.md) | founding package for the Idea Engine (Q-0261 seat 4) — **v2, rewritten per owner ruling Q-0264**: own `idea-engine` repo sectioned per fleet lane (parallel agents), probe battery v0, harvest-by-link, sim-ready ideas marked for the Simulator's direct pull; finalizes nothing | router Q-0264 · `idea-probe-brainstorm-simulator` (`../ideas/idea-probe-brainstorm-simulator-2026-07-10.md`) |
| [round3-founding-package-simulator](round3-founding-package-simulator-2026-07-10.md) | founding package for the Simulator Project (`sim-lab`, **core seat 6 per Q-0264** — supersedes the Q-0262.8 hub pick): the fleet's evidence stage — cheapest-adequate-method sims/prototypes, the five-question validity gate, @codex review before finalization, reusable harness as a public product; manager final-reviews + routes | router Q-0264 · precedents `tools/sim/claim_layout_sim.py` + `tools/sim/gen3_deployment_sim.py` |
| [round3-founding-package-product-forge](round3-founding-package-product-forge-2026-07-10.md) | founding package for the Product Forge (standing-core seat 4): new `product-forge` repo born-right (gen2-blueprint §1 as ORDER 000), one-product-one-subtree, money protocol per Q-0259 r.4 | [round3-launch-pack](round3-launch-pack-2026-07-10.md) §5 |
| [round3-dispatch-next-session-brief](round3-dispatch-next-session-brief-2026-07-11.md) | the part-4h → next-session handoff: verified fleet roster, the remaining finalization-verification items (games boots close the loop, venture-lab relaunch, manager relay adoption, first @codex proof, Pages live), the owner-click tail, and the Q-0268 autonomy posture | router Q-0268 · runbook §5/§7 |
| [round3-founding-package-games-world](round3-founding-package-games-world-2026-07-10.md) | founding package for the games program's Seat A (**Q-0267**): gen-2 SINGLE seat on `superbot-games` merging the two terminal gen-1 lanes — world ecosystem (exploration + mining + fishing + shared systems), unification slice as walking skeleton, integrity floor kept (deterministic core, sim-pinned balance, no pay-to-win, bounded-menu AI DM), merge-ladder from the repo's own retro walls, kit v1.2.0→v1.7.0 queued | router Q-0267 · `../ideas/games-theme-engine-website-first-2026-07-10.md` |
| [round3-founding-package-games-idle](round3-founding-package-games-idle-2026-07-10.md) | founding package for the games program's Seat B (**Q-0267**): NEW repo (suggested `superbot-idle`) seeded on the sim-lab recipe — idle-engine core + data-only theme packs (egg farm first), CORE/SKIN split as the repo's hard rule, theme-gate CI, setup-code provisioning format for website-first onboarding, economy numbers pre-registered + sim-lab-routed | router Q-0267 · `../ideas/games-theme-engine-website-first-2026-07-10.md` |
| [round3-founding-package-trading](round3-founding-package-trading-2026-07-10.md) | founding package converting the trading lane (`trading-strategy`) into a standing continuous seat (owner-directed part-4b, superseding the "after the holdout ORDER" sequencing): **ORDER-008-first boot** (the fresh context satisfies the protocol's dedicated-session condition), both verdict branches pre-specified, integrity contract + money protocol in the instructions, volume-first per **Q-0266**, routine cutover create-verify-then-delete, kit v1.1.0→v1.7.0 queued | router Q-0250/Q-0251/Q-0265/**Q-0266** · `trading-strategy` `docs/p5-holdout-protocol.md` |
...
| [round3-dispatch-part4-brief](round3-dispatch-part4-brief-2026-07-10.md) | successor brief from dispatch part 3 (PR #1955): paste-ready part-4 opener, state snapshot (seats 1–4 LIVE, 5 seeded, 6 pending), priority queue incl. the forge ORDER-001 games-web dispatch paste + the sim-lab boot (closes the Q-0264 loop on PROPOSAL 001), the twice-proven fleet-repo seeding recipe | [round3-dispatch-runbook](round3-dispatch-runbook-2026-07-10.md) §3/§5 |
...
Owner-directed initiative for a from-scratch SuperBot rebuild "designed as one picture," with the
current repo as the what/why/how record and the substrate-kit finished first as its foundation.
**The rebuild is UN-GATED (Q-0241, #1776, 2026-07-07):** the go/no-go sitting and every owner gate
are retired — the coordinator builds in logical order, live-tests in a real server, and never waits
(silence = consent); the owner's control is reacting to the flagged decisions + what he sees live.
...
| [**rebuild-project-kickoff**](rebuild-project-kickoff-2026-07-08.md) | **the production Project kickoff (2026-07-08)** — paste-in Custom Instructions + startup prompt for the fresh Project that autonomously builds canonical-plan §5 **steps 7–13** (populate `substrate-kit` → `superbot-next` adopts from it → kernel K0→K8 → K9+strand-3 → layer-V files → K10 → port bands), **forward-only / build-first / test-later**; owner-only + destructive steps (14–17, repo settings, Railway) are flagged, never executed. Both target repos are already seeded, so normal forward-only git (branch → PR → squash-merge) applies |
| [**rebuild-direction-handoff**](rebuild-direction-handoff-2026-07-08.md) | **the management-layer handoff (2026-07-08)** — hands the "read the coordinator's status reports, guide the owner through owner-only steps, score the two EAP axes" role to the directing session; live-state pointer + the owner-only checklist + banked findings |
| [**rebuild-project-audit-checklist**](rebuild-project-audit-checklist-2026-07-08.md) | **the repeatable correctness audit (2026-07-08)** — what to check on every coordinator status report (directing quality, spec-correctness, what's-been-tested tracking, repo hygiene) vs. the deeper ~every-10-PR spot-check; grew out of catching a re-asked router question and an unfilled `current-state.md` in `superbot-next` |
| [~~rebuild-status-site-project-kickoff~~](rebuild-status-site-project-kickoff-2026-07-08.md) | **SUPERSEDED 2026-07-09** by [websites-project-kickoff](websites-project-kickoff-2026-07-09.md) — kept for the reasoning trail only |
| [**websites-project-kickoff**](websites-project-kickoff-2026-07-09.md) | **a third, separate Project's kickoff (2026-07-09)** — `menno420/websites` (owner-created, empty) as the **permanent consolidated home for all websites**, a sibling rebuild track to the bot rebuild (substrate-kit foundation, keep-the-ideas/rebuild-the-implementation). Step 1: adopt substrate-kit. Step 2 (first real deliverable): a control-plane oversight site — per-repo readiness board (rulesets/checks/CODEOWNERS/secrets/auto-merge, seeded from [repo-settings-state](../operations/repo-settings-state.md)) + a cross-repo journal/decision browser. Step 3 (named, deferred, owner-checkpoint required): rework `dashboard`/`botsite` into this repo — real work, not started autonomously. New Railway project; owner-accepted full-access token + the one hard destructive-mutation rail carried over unchanged |
...
| [**eap-project-fleet**](eap-project-fleet-2026-07-09.md) | **broad capability + model-comparison test plan (2026-07-09)** — prep for a dedicated next session: 7 domain-breadth Projects (games/bots/research/coding/design/personal/wildcard) as an extensive multi-axis EAP eval, a 3-model comparison sub-experiment anchored on the coding task (Fable 5 / Opus 4.8 / Sonnet 5, identical brief), and the already-planned substrate-kit lab as the 11th. Shared Custom Instructions template + a next-session execution checklist; feeds the Friday evidence package |
| [**eap-manager-project-brief**](eap-manager-project-brief-2026-07-09.md) | **the cross-repo oversight/direction manager Project (2026-07-09)** — owner-directed founding brief for a manager Project that sits *above* the build Projects (coordinator/kit-lab/websites/test-fleet): ingests their reports + committed state, audits via the [audit checklist](rebuild-project-audit-checklist-2026-07-08.md), surfaces cross-Project patterns, and produces one owner-facing rollup; leans on the `websites` control-plane board as the live dashboard rather than reinventing tracking. Paste-ready Custom Instructions + calibration. Companion to the [fleet review](../eap/fleet-review-2026-07-09.md) |
| [**fleet-coordination-protocol**](fleet-coordination-protocol-2026-07-09.md) | **the manager Project v2 + git-as-message-bus (2026-07-09)** — owner-directed: one manager Project as the owner's single control chair over the whole fleet, and a file-based inter-Project coordination protocol (shipped in the substrate-kit) so Projects coordinate through committed `control/inbox.md`/`status.md` files (one writer per file; distributed own-repo; self-poll routines; autonomous-director manager). Paste-ready activation messages + phasing. Upgrades the observe-only [manager brief](eap-manager-project-brief-2026-07-09.md) to observe+dispatch |
| [**manager-project-kickoff**](manager-project-kickoff-2026-07-09.md) | **paste-ready launch materials for the manager Project (2026-07-09)** — the durable Custom Instructions (Project settings), a one-time start-off prompt that reviews all repos + plants the `control/` files + seeds a first status explanation + builds `fleet-manifest.md` + emits the per-project init prompt, and the small per-Project init prompt. Operationalizes the [manager brief](eap-manager-project-brief-2026-07-09.md) + [coordination protocol](fleet-coordination-protocol-2026-07-09.md) |
| [**fleet-retro-questions**](fleet-retro-questions-2026-07-09.md) | **the gen-1 fleet self-review protocol (2026-07-09)** — manager-authored question set (work/correctness · errors/friction · efficiency · autonomy · protocol/environment · redesign, plus per-repo addenda) that every gen-1 fleet Project answers in its own repo at `docs/retro/self-review-2026-07-09.md` via an inbox ORDER; honest > flattering, claims tied to git evidence. Answers feed the gen-2 Project-setup blueprint |
...
| [**rebuild-final-review-fable5-ultracode-brief**](rebuild-final-review-fable5-ultracode-brief-2026-07-07.md) | **⭐ paste-ready Fable-5 `/effort ultracode` launch brief (2026-07-07, PR #1777)** for the **FINAL review of the rebuild plan** — reading route + "already-settled don't-redo" baseline + the A–H mandate (plan review + readiness score, feature-vs-plan gap hunt, un-added ideas, forgotten/stale prose, Q-0241 review, a **Projects-EAP product review** for Anthropic, simulator grouping/naming centralization, active repo-prep + live-bot fixes) + a ranked re-verify checklist. Runs under Q-0241 (never-wait, live-test, silence=consent) |
| [rebuild-idea-consolidation-fable5-ultracode-brief](rebuild-idea-consolidation-fable5-ultracode-brief-2026-07-07.md) | **paste-ready Fable-5 `/effort ultracode` launch brief (2026-07-07)** — a narrower follow-on to the final review: fold five owner-raised, same-day findings into the canonical plan (the **K6 channel-role-scoped authority gap**, time-sensitive since K6 isn't built yet; the **K9 user-self-service automation scheduler**, category-B gated pending its own pricing session; judgment calls on the moderation and guild-config-backup/GDPR-export gaps) and re-verify the plan against five specific critique points (layer-V depth enforcement, AI-verification gate requirement for its port band, the human `verified_live` time budget, ratchet permanence, rollback-window-vs-cadence). Explicitly excludes the automod duplicate/cross-channel-spam finding (a live-bot track, not the rebuild plan) |
...
| [rebuild-phase-2.5-procedure](rebuild-phase-2.5-procedure-2026-07-06.md) | **companion D** — the runnable Phase-2.5 substrate-on/off cold-start A/B (environment, arm protocol, 3 measures, pass bar F-5, artifact); prereq kit tail ① ✅ shipped PR #1775 |
| [phase-2.5-cold-start-report](phase-2.5-cold-start-report-2026-07-07.md) | **the G2 evidence (RUN 2026-07-07, PR #1775)** — 8 paired sessions + independent judge; verdict **FAIL as-tested**: `adopt` ships the kit inert (unrendered `${...}` templates → orientation cost in 3/4 pairs, no measured benefit); recommended ruling = adopt-renders-what-it-knows fix + one re-run pair — **executed 2026-07-07** (see the final-review report); verdict-acceptance is agent-side under Q-0241 |
| [rebuild-design-spec](rebuild-design-spec-2026-07-02.md) | **the Phase-2 design spec.** **Superseded-in-part (canonical plan §9):** §9.1 K9/K10 + §9.2 port order → the Gate-0 build order + Sequence C; §2 grammar → the Gate-0 frozen grammar + 14 specs where they differ. Its §10.2 ratification list is now the **decide-and-flag veto payload** (canonical §1, F-2/F-3) — the G1 *sitting* it awaited was **retired by Q-0241 (#1776)**; the owner vetoes items reactively, nothing blocks. The "one picture": architecture + manifest grammar (simulable, S/A/O-classified) + central namespace + settings model (safe-default-ON) + data/backward-compat contract + control plane + build order |
| [fresh-rebuild-strategy](fresh-rebuild-strategy-2026-07-02.md) | the **verified baseline**; analysis-grade. **§3 arc + §3.1 model table superseded by the canonical plan §3** (Phase 0 ✅ #1649 · 0.5 ✅ #1639 · 1 ✅ · 2 ✅); §1/§4/§5/§6 stay reference |
| [rebuild-parallel-execution-plan](rebuild-parallel-execution-plan-2026-07-02.md) | **superseded by the canonical plan §4/§5** (its "two gates" both cleared; its K4–K6 off-spine concurrency claim corrected — D-19); §1's measured velocity baseline stays citable data |
...
| [rebuild-stage2-subsystem-walk](rebuild-stage2-subsystem-walk-2026-07-05.md) | **`plan` (living) — the Stage-2 subsystem walk itself (owner-led, started 2026-07-05).** The canonical progress index (52 rows: 43 shipped BUILD-PLAN subsystems + 9 ADD rows, mapped from all 58 live cogs) plus the non-cog platform queue and the per-subsystem durable records as they're decided. Continue this file — do not create a second Stage-2 walk artifact. |
...
| [rebuild-gate-v-verification-fleet](rebuild-gate-v-verification-fleet-2026-07-06.md) | **the GATE V launch pad (2026-07-06)** — paste-ready, capability-safe prompts for the verification-fleet pass (Q-0234) between Phase A and Phase B: four independent arms (Sonnet-5/Ultracode architecture · a **5-session Codex fan-out C1–C5** source/test truth · Agent Mode external/migration/live-GitHub · an empirical **live-testing** arm that captures the `verified_live` goldens the paper reviews can't) reconciled by a final Opus/Fable synthesis, with shared enum / evidence-label / claim-anchor contracts so the four reports merge without manual normalization |
| [rebuild-newrepo-start-fable5-ultracode-brief](rebuild-newrepo-start-fable5-ultracode-brief-2026-07-06.md) | **paste-ready Fable 5 Ultracode launch brief (2026-07-06)** to **finalize the method of starting the new repo** (post-Gate-V): reviews the finished substrate-kit + bot code + plans + simulators + test strategy, reconciles the two phase vocabularies into one canonical start arc, and produces four deliverables — the ordered start method, the owner-decision packet (Gate-0 rows · backward-compat contract · manifest format · F-3 · rollback window), a **per-function test-guild design** (~55 cogs → channel zones + full-command-pipeline fidelity), and a runnable **Phase-2.5 cold-start** procedure. Fable-tuned (goal+constraints+grounding, async sub-agents, xhigh); finalize-not-build |
| [rebuild-gate-v-findings-corrections](rebuild-gate-v-findings-corrections-2026-07-06.md) | **the verified evidence layer for Gate V synthesis (2026-07-06)** — re-verifies the four Codex Arm-B sub-report PRs (#1752/#1753/#1754/#1755, all **sound**) and the Agent Mode report against live source: records the **C1 (L0/runtime) failed-to-start gap** + re-run prompt, the Agent Mode **`ci-gate` error** the synthesis must reject (live gate is `code-quality`), the all-`source-read`-not-`test-confirmed` caveat, and the substantive contract-freeze findings (economy/XP event atomicity, channel ownership gap, `ai`/`btd6` zero L3 dependency). Arm Σ consumes this instead of the raw sub-reports |
| [SONNET-5-ULTRACODE-CORE-READINESS-REVIEW](../analysis/rebuild-discovery/gate-v/SONNET-5-ULTRACODE-CORE-READINESS-REVIEW.md) | **`audit` — Gate V Arm A (Sonnet-5/Ultracode architecture & readiness review, 2026-07-06)** — PRIMARY owner of the sequencing verdict, architecture-invariant pressure-test, and games-deferral design logic; produced by an 11-agent internal fleet (4 readiness lenses + 3 PRIMARY drafts + 3 independent adversarial verifications). Key finding: the two live money bugs cited as the rebuild's strongest justification for early kernel work (K7) are fixable today via an already-adopted mixin, independent of any sequencing decision; recommends **Sequence C** (capability-class, bounded interleave) over the frozen L3-before-L4/L5 order, for which no dependency edge was found anywhere in source. Surfaces two capstone-accuracy contradictions beyond the Stage-2 walk's known five (the visual card engine and the boards family are both already built). Arm Σ consumes this alongside the Codex/Agent-Mode/live-testing evidence |
