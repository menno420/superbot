...
| [per-repo-settings-state-ledger](per-repo-settings-state-ledger-2026-07-08.md) | **owner-raised (2026-07-08, EAP-email thread, PR #1843)** — a per-repo settings/state ledger (required checks, auto-merge, rulesets, token surfaces, permission-wall behavior) so sessions *read* repo configuration instead of guessing it; pairs with the forward-only Project quality experiment idea. *(Homed 2026-07-08 by lane B — landed unindexed in #1843 and reddened `check_plan_homing`.)* |
