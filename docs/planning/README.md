...
| [**rebuild-status-site-project-kickoff**](rebuild-status-site-project-kickoff-2026-07-08.md) | **a second, separate Project's kickoff (2026-07-08)** — build + deploy a live rebuild-status site (PR velocity, decision ledger, open router items, parity ratio across `superbot-next`/`substrate-kit`) in a brand-new repo + Railway project, deliberately overlapping `botsite/console` to test end-to-end repo/infra provisioning from a Project — the first test of Railway access from a Project; owner accepted using the existing full-access account token rather than a scoped one |
