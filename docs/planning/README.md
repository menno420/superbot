...
|---|---|---|
| [consolidation-discoverability-audit-brief](consolidation-discoverability-audit-brief-2026-06-23.md) | **▶ next priority (owner-directed)** — staging brief for a full per-cog consolidation/discoverability audit: every command findable + buttonized, setup wizard improved, AI advisor finalized, per-cog + general settings centralized. Reconciles the stale-status findings below; carries the per-cog rubric + first targets. | cross-cutting · `competitive-positioning-north-star` |
