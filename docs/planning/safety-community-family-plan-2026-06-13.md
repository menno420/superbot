...
| **welcome v1 + counters** ✅ *(shipped — slot 6, #775)* | Q-0110 | `services/welcome_service.py` + `welcome_config.py` (embed-only, entry-role via audited `role_automation`) + `services/counter_service.py` + `counter_config.py` (statdock renames) — **two hub-less new subsystems** | `mock_welcome_ab`, `mock_counters` | join/leave embed + entry-role; counters = periodic channel-rename loop (10-min cadence + change-detection, respects the 2/10-min cap). Both default OFF; no migration. |
| **image moderation** ✅ *(shipped — PR #941, `needs-hermes-review`)* | Q-0108 | `services/image_moderation_service.py` (pure) + `services/image_moderation_config.py` + `core/runtime/ai/providers/openai_moderation.py` (the SDK chokepoint) + `cogs/image_moderation/` — the order-25 `ImageModerationStage` | (reuse automod panel) | OpenAI `omni-moderation-latest` **only** (free, existing key); paid tiers declined; four buckets (sexual·violence·harassment·hate), threshold ≥0.80; off by default · fail-open · only the image URL leaves the bot (disclosed in the master-switch hint) |
| **security tiers 1+2** ✅ *(shipped — slot 9, #929)* | Q-0111 | `services/security_service.py` + `security_config.py` + `cogs/security/` + `cogs/security_cog.py` — a new hub-less subsystem (raid detection `RaidTracker` + account-age filter) | `mock_security_alerts` | tiers 3+4 (alt-detection / VPN) **declined** — kept absent; self-contained, no external API. Off by default; raid lockdown raises slowmode + staff alert (deduped, auto-restored); age filter alerts or kicks (kick via `moderation_service`). No migration. |
