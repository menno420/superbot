...
| Interaction router "fails open" when governance resolution throws | A | Behavior plausible from source; exact `dispatch()` failure path **not** line-verified this pass. Carried as a **policy decision** (fail-open vs fail-closed per surface), not a confirmed bug. |
...
| RC-4 | Authority not capability-native; unconsulted flags | B | confirmed | high | BI (settings UI expansion) | GV | `settings_mutation.py:32-33,75-78,423-451`; `binding_mutation` no-op; ledger §1 | Wire typed capability before broadening settings/bindings surfaces; decide flag semantics. |
...
| RC-10 | BTD6 provenance + ownership matrix + freshness | D | partially confirmed | high | BX | BD+MD | many `services/btd6_*`; provider comments | Decisions doc: owner per fact-type + one provenance object; **pause extraction** until set. |
| RC-11 | AI central orchestration (preserve) | D | confirmed-healthy | n/a | BA (guardrail) | BD | `ai_tools.py`, `natural_language_stage`, policy resolver | Do **not** refactor the choke point; add tests before expanding AI features. |
| RC-12 | YouTube/media ownership ambiguity | D,Codex | needs maintainer decision | low | NB | MD | `youtube_*` services; cartography §13 | Decide: shared `video_reference`/media subsystem (not BTD6). |
