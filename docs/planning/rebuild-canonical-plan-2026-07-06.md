...
| ⚠ Q-D5 | intent posture | **(a) DEGRADE** — flip the frozen `required=True` floor at this sitting (the design recommendation, diverging from the shipped fail-closed floor; = open fork F-3/PG-2). **+ Q-0246 rider (owner ruling, 2026-07-07): degrade is also a supported PRODUCT tier — server owners choose Full (admin) vs Lite (no elevated permissions) at setup, features degrade visibly; §11b A-22 / rider R-18** | slash-first survivability (Q-D21) is incoherent with a bot that refuses to boot on intent denial; this sitting IS the PG-2 ruling the flip waits on |
...
| 6 | ✅ **Repo created** (`superbot-next`) — owner-created 2026-07-07, empty; **deliberately PUBLIC for now** (unlimited free Actions minutes — owner directive in-session 2026-07-07, amending this row's original "private"); **flip-to-private is a checkpoint at step 8 and HARD before step 15** (CUT-2 dry-run artifacts carry real user balances; the flip is also the free-Actions cost checkpoint) | owner (done) | steps 7+ run through the Projects coordinator — send order, Custom Instructions, calibration exchange + kickoff in [`projects-eap-coordinator-kickoff-2026-07-07.md`](projects-eap-coordinator-kickoff-2026-07-07.md); the **2026-07-08 production kickoff** (paste-in Custom Instructions + startup prompt for steps 7–13, forward-only / build-first) is [`rebuild-project-kickoff-2026-07-08.md`](rebuild-project-kickoff-2026-07-08.md) |
| 7 | **Extract the kit repo, *then* bootstrap** (Q-0247, amending this row's original direct-adopt phrasing): (a) extract `superbot/substrate-kit/` into its **own repo `menno420/substrate-kit`** — the kit's permanent home and the source every future repo adopts from (kit-lab, trading); (b) **`superbot-next` adopts FROM that repo** via `python3 dist/bootstrap.py adopt` → doc skeletons, decision ledger, orientation-budget checker, namespace/seam checkers, staged hooks. **Fresh-from-kit, never a copy of superbot**; the kit is not adopted directly out of superbot's directory. | agent (coordinator) | K0's first act; adopt re-proven live this session (17 planted + 14 staged artifacts, `check --strict` clean); full packaging = [steps-6–8 brief](rebuild-kickoff-steps-6-8-brief-2026-07-07.md) |
...
**Deliberately NOT folded** (rationale in the report): `check_doc_cites.py` (current-repo doc hygiene — routes as its own idea/PR, gates nothing here); Projects-EAP as coordinator (the plan stays product-agnostic — **the thin wiring note landed 2026-07-07**: [`projects-eap-coordinator-kickoff-2026-07-07.md`](projects-eap-coordinator-kickoff-2026-07-07.md) — repo scope, Custom Instructions, and the first kickoff message for running §5 step 6+ through a Claude Code Project; **revised the same evening** into the full handoff protocol: the owner→coordinator *calibration exchange* sent before any work, the Project-created/UI facts incl. the step-6 public-repo amendment, and the Q-0247 step-7 fold; this plan itself is unchanged, the note is execution-mechanism only); C-7 one-description-surface (already structurally covered: K2 manifest + help-as-projection + manifest-generated intent surface); a standalone START-HERE index (this plan + §9 already are it).
...
| A-21 | **Test-driver account (owner ruling Q-0245, 2026-07-07)** — the owner's second account is a **declared elevated test actor**: env-declared (`EXTRA_OWNER_USER_IDS`, comma-separated), never source-hardcoded, clearing **both** owner seams together (`config.is_platform_owner()` — the canonical single-source predicate — and `bot.is_owner()` via a thin delegating override). **Shipped on the live bot the same day** (this PR): empty default = zero behavior change until the owner sets the variable; 8 test pins. New-bot landing: a config-declared extra-owner set recognized by K6 `resolve_authority`'s owner predicate; companion C's lane-B driver account is its first consumer (moderator-function walks without real Discord roles). ⚑ Safety flag: an id in the set is a full-power operator credential in **every** guild — the owner treats that account's security like the main account's | live bot (shipped) + K0/K6 + companion C §4 |
| A-22 | **Permission-tiered operation (owner ruling Q-0246, 2026-07-07)** — server owners choose **Full** (administrator, everything) vs **Lite** (games/community/utility needing no elevated permissions; moderation/channel/role/cleanup stay dark), rider **R-18**: every feature's manifest declares its required Discord permissions → the compiler derives the two invite URLs/permission bitmasks (presented at setup) + the runtime permission census; a feature whose permission is absent degrades **visibly** ("needs Manage Roles" in help/setup — never silent failure or crash). This **extends the Q-D5 DEGRADE ruling from failure posture to a supported product configuration** — running without admin is a first-class mode, the "failsafe for servers that don't want to grant permissions". Lite→Full = re-invite/grant + census flip, no config/data loss; gateway *intents* keep their separate Q-D5/PG-2 treatment. Exact grammar carrier (widen `ResourceRequirement` vs a new spec field) decided at the owning K2/K6 fold per R-18; the A-2 schema-growth ledger applies if a new field is minted | K2/K6 + S9 (setup) + §1 F-2 Q-D5 rider |
