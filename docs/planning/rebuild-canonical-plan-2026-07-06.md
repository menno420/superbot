...
**Deliberately NOT folded** (rationale in the report): `check_doc_cites.py` (current-repo doc hygiene — routes as its own idea/PR, gates nothing here); Projects-EAP as coordinator (the plan stays product-agnostic — **the thin wiring note landed 2026-07-07**: [`projects-eap-coordinator-kickoff-2026-07-07.md`](projects-eap-coordinator-kickoff-2026-07-07.md) — repo scope, Custom Instructions, and the first kickoff message for running §5 step 6+ through a Claude Code Project; this plan itself is unchanged, the note is execution-mechanism only); C-7 one-description-surface (already structurally covered: K2 manifest + help-as-projection + manifest-generated intent surface); a standalone START-HERE index (this plan + §9 already are it).
