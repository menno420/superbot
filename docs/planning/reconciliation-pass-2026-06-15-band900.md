...
| 7 | **Safety lane — welcome phase 2 (PIL cards)** | ✅ DONE (PR #920) | Q-0110 — `welcome_card_enabled` toggle (off by default) attaches a PIL greeting card to the join embed; the prototype graduated to `utils/welcome_render.py` (gallery re-exports it); degrades to embed-only without Pillow |
