...
| Channel permission overwrites | `ChannelCog.set_access`; `lock_channel`; `unlock_channel`; `modify_permissions`; `disbot/views/channels/restrict_panel.py`; `visibility_panel.py` | Management functions / panels | **Partial** | Functional, but overwrite mutations remain outside `ChannelLifecycleService`; the channel invariant explicitly defers pinning `.set_permissions()`. | Source inspection and invariant scope note. |
| Channel clone | `ChannelCog.clone_channel` | Management function | **Partial** | Functional direct clone path remains outside the canonical lifecycle service and invariant. | Source inspection and invariant scope note. |
...
1. **Converge the remaining channel mutation paths.** Direct creation, clone, permission-overwrite, and category lifecycle paths need one declared canonical audited owner before the subsystem can be called production-ready end to end. Preserve the resource-provisioning confirmation rule for creation; do not simply widen `ChannelLifecycleService` without deciding ownership.
2. **Expand the channel mutation invariant only after convergence.** Pin `.set_permissions()`, `.clone()`, and creation/category calls once their canonical service path exists. The current invariant intentionally leaves these gaps open.
