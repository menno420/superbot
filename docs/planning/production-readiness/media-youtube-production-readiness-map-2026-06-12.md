...
| Fetch/cache/DB/migration focused tests | `tests/unit/services/test_youtube_fetch_service.py`; `tests/unit/services/test_video_reference_cache_service.py`; `tests/unit/utils/db/test_youtube_video_cache_stats.py` | Tests | **Partial** | `youtube_fetch_service` now has a dedicated test (URL parsing, the full metadata-fetch error taxonomy + the content-free provider-outcome it records, transcript degrade-to-empty — all I/O mocked, 2026-06-27); `video_reference_cache_service` and the `youtube_video_cache` stats primitive also have focused tests. Still uncovered: migration `049` directly and broader cache-DB semantics (TTL/expiry rows, error-cache parity, concurrency). | 2026-06-27 dispatch run. |
