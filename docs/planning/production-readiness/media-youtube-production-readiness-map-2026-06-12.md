...
| Fetch/cache/DB/migration focused tests | `tests/unit/services/test_youtube_fetch_service.py`; `tests/unit/services/test_video_reference_cache_service.py`; `tests/unit/utils/db/test_youtube_video_cache.py`; `tests/unit/utils/db/test_youtube_video_cache_stats.py` | Tests | **Partial** | `youtube_fetch_service` has a dedicated test (URL parsing, the full metadata-fetch error taxonomy + the content-free provider-outcome it records, transcript degrade-to-empty — all I/O mocked, 2026-06-27); the `youtube_video_cache` DB primitives are now tested (`get_video_cache` incl. the `expires_at > now()` TTL read filter · `upsert_video_cache` param/JSON/`$2::jsonb`/error-cache contract · `purge_expired_video_cache` `DELETE N` parse · plus the existing content-free `get_cache_stats` test), pool-mocked offline (2026-06-27); `video_reference_cache_service` also has focused tests. Remaining: migration `049` *application* itself is a live-DB concern (`[needs-live-bot]`) — the only un-offline-testable part of this row. | 2026-06-27 dispatch run (slices 1+2). |
| Embed/renderer focused tests | `tests/unit/views/test_youtube_embeds.py`; `tests/unit/views/test_youtube_renderers.py` | Tests | **Done** | Direct tests for the card + compare embed builders (metadata, duration formatting incl. hours, fallbacks, truncation) and the `render_describe` / `render_compare` renderers (wrong-context-type + too-few-videos guards + happy path). Added 2026-06-27 dispatch run. | 16 focused view tests. |
