...
> **Status:** `historical` · **Phase 0 complete; Phase 1 in progress** — P0A (Q-0026 identity repair), P0B (direct-vs-draft + access read-model contracts), and **P0C (role-threshold writer normalization)** all done 2026-06-08; **P1A (Access Map projection service)** done; **P1B complete** (remainder shipped 2026-06-09 in **#632**: governance tier-input path + `help_advertises_locked` + the Q-0036 denial-copy *draft* — wiring stays gated on the maintainer's markup); **P1C next** (Batch 5 of [`consolidated-implementation-plan-2026-06-10.md`](consolidated-implementation-plan-2026-06-10.md)) · **Source-verified 2026-06-08; status updated 2026-06-10**
> **Superseded 2026-06-19 (was active):** P0 + P1A/B/C shipped (#588/#589/#591/#592/#632/#656). Do not act on this — current map: [planning/README](README.md).
...
| **P1B — Sonnet:** drift + locked reasons | ✅ **DONE (re-scoped §16.8 items 5–7).** `routing_access_conflict` built (#592, member-independent, 7 tests); **remainder shipped 2026-06-09 in #632** — governance tier-input path (Q-0045) + `help_advertises_locked` + the Q-0036 denial-copy **draft** (PR-body review; live wiring gated on the maintainer's markup). **Skipped `configured_resource_missing`** — already covered by the four existing collectors. | `setup_diagnostics.py` (new `_diagnose_*` collectors), tests | Editing; `configured_resource_missing`; silent denial-message changes | Diagnostics/access tests + quality | Medium; stop if explanation leaks sensitive policy or a provider needs audience sim that isn't built. |
| **P1C — Sonnet:** Access Map + Help Preview UI | ✅ **DONE 2026-06-10** (consolidated plan Batch 5, **#656** — verify merged): both staff-hub subpanels shipped in `views/server_management/access_map.py` over `project_access_map` (Q-0032: hub buttons only, no new command names; Q-0045 declared-tier simulation + §16.4 limit label on every rendering; authority re-checked per interaction; display-only pinned by a mutation-import test; live-smoked). | server-management/help/views/cogs/tests | Mutation | View/authority/help tests + live smoke | Medium. |
...
3. ✅ **P1A — DONE (2026-06-08):** the side-effect-free Access Map projection service (`services/access_projection.py`) + 19 tests per §16 (no UI, no persistence). ✅ **P0C — DONE (2026-06-08, #592):** all six role-threshold write sites route through the audited `role_automation.set_{time,xp}_threshold` seam; drift-fence allowlist emptied (§16.5). ✅ **P1B `routing_access_conflict` — DONE (#592)**; **skipped** `configured_resource_missing` (already covered by the four existing collectors — §9 row). ✅ **P1B remainder — DONE (2026-06-09, #632):** the governance tier-input path (**Q-0045**) + `help_advertises_locked` + the **Q-0036** denial-copy **draft** (live wiring stays gated on the maintainer's markup of the #632 table). **Next (updated 2026-06-10): P1C** (read-only Access Map + Help Preview, as **staff-hub subpanels only — no new command names**, Q-0032) — Batch 5 of [`consolidated-implementation-plan-2026-06-10.md`](consolidated-implementation-plan-2026-06-10.md). Read §16.8 before P1C.
