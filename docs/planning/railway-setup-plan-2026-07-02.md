...
| **Alerts** | Workspace usage limit + email alert (soft $15 set 2026-07-02); deploy/volume/monitor notifications → Discord **verified live 2026-07-02**: the mechanism is a **notification rule** (`notificationRules`/`notificationRuleUpdate`, `channelConfigs: [{type: "webhook", url: <discord webhook>}]`, `eventTypes: Deployment.*/VolumeAlert.*/Monitor.*`) — the current project's rule posts to `#railway-alerts` in the test guild (restored after accidental channel deletion); the new project mints its own rule at bootstrap, destination = the HQ guild (see `../ideas/central-admin-and-logging-guilds-2026-07-02.md`). |
