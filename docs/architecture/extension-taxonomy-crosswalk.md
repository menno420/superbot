...
_Generator:_ `scripts/extension_crosswalk.py` `v1`  ·  **49** extensions  ·  **37** registered subsystems  ·  **12** non-1:1 extensions.
...
| `product_subsystem` | 28 | A feature vertical with its own owner, views, and tests. |
...
| 23 | `creature_battle` | `product_subsystem` | — |  | Creature PvP battle cog (!cbattle) — part of the Creatures subsystem (not a separate registered subsystem; surfaced via creature_cog's Help hook). |
| 24 | `diagnostic` | `shared_platform` | ✓ |  | `!platform` diagnostics; broad cross-subsystem read-model surface. |
| 25 | `health_maintenance` | `maintenance` | — | `diagnostic` | Scheduled health-findings retention loop; no subsystem identity. |
| 26 | `ai` | `shared_platform` | ✓ |  | AI orchestration / answerability; cross-cutting. |
| 27 | `media_maintenance` | `maintenance` | — |  | Scheduled media/YouTube cache purge loop; no subsystem identity. |
| 28 | `btd6` | `product_subsystem` | ✓ |  | BTD6 core data / answerability vertical. |
| 29 | `btd6_reference` | `specialized_surface` | — | `btd6` | BTD6 reference lookups. |
| 30 | `btd6_events` | `specialized_surface` | — | `btd6` | BTD6 live-events surface. |
| 31 | `btd6_strategy` | `specialized_surface` | — | `btd6` |  |
| 32 | `paragon` | `specialized_surface` | — | `btd6` | BTD6 paragon grounding surface. |
| 33 | `btd6_ops` | `specialized_surface` | — | `btd6` | BTD6 data-ops (seed/refresh); operational flavor within the BTD6 vertical. |
| 34 | `chain` | `product_subsystem` | ✓ |  | Command-chain subsystem. |
| 35 | `general` | `product_subsystem` | ✓ |  | General-content commands. |
| 36 | `four_twenty` | `product_subsystem` | ✓ |  | Novelty / community feature. |
| 37 | `leaderboard` | `product_subsystem` | ✓ |  | Cross-subsystem leaderboards (reads XP/games). |
| 38 | `settings` | `shared_platform` | ✓ |  | Settings hub; cross-cutting config surface. |
| 39 | `logging` | `product_subsystem` | ✓ |  | Server-logging subsystem. |
| 40 | `games` | `hub` | ✓ |  | Games hub (routes blackjack/deathmatch/counting/rps). |
| 41 | `community` | `hub` | ✓ |  | Community hub. |
| 42 | `community_spotlight` | `product_subsystem` | ✓ |  | Community Spotlight (community-hub child; registered subsystem). |
| 43 | `welcome` | `product_subsystem` | ✓ |  | Welcome service (join embeds + optional PIL cards). |
| 44 | `counters` | `product_subsystem` | ✓ |  | Dynamic server counters. |
| 45 | `security` | `product_subsystem` | ✓ |  | Security tiers 1+2 — raid detection + account-age filter (Q-0111). |
| 46 | `setup` | `bootstrap` | — | `server_management` | Guided setup wizard; lifecycle-critical, load-order sensitive. |
| 47 | `server_management` | `hub` | ✓ |  | Routing-only hub (moderation/channels/roles/cleanup/setup); holds no capability of its own. |
| 48 | `hermes` | `operational_adapter` | — |  | Bridge to the Hermes control plane / external operation. |
| 49 | `ux_lab` | `lab` | ✓ |  | Zero-write UX pattern gallery (admin-gated); design vocabulary, not a product surface. |
