...
| **Channel lifecycle** | `services/channel_lifecycle_service.py` | channel cog (typed commands) + `views/channels/move_panel.py` | `ChannelLifecycleService().apply(ChannelLifecycleRequest(...))` — `rename` / `move` / `delete` / `reorder` (top/bottom) | best-effort `audit.action_recorded` companion + `channel.lifecycle_changed` event (shared `mutation_id`); no dedicated audit table | create/clone/overwrites/category-CRUD not yet routed — pinned (`.delete`/`.edit`) by `test_no_direct_channel_mutations.py` |
