...
| `docs/owner/maintainer-question-router.md` | **Append-only.** Add the next free `Q-00NN` block at the end; never renumber or reflow existing ones. **Merge collision** (two sessions appended concurrently — it happened 2026-06-09): the *merged/answered* entries keep their numbers; an unmerged entry that duplicates an answered question is **dropped**, not double-recorded; still-open unmerged entries **renumber to the new tail** and the renumbering session fixes its own cross-references. **Answer scope:** when recording an answer next to adjacent mechanics, add one line on what it does *not* decide (the Q-0050 "craft-once" wording forced a re-ask, Q-0054). |
