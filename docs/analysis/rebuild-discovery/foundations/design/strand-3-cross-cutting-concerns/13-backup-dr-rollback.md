...
|---|---|---|
| `REVERSE_IMPORTABLE`, **LEDGER** rows (e.g. `economy_audit_log`, `audit_log`) | **re-insert each post-`cutover_flip_ts` row into the OLD DB by its `mutation_id`** (`INSERT … ON CONFLICT (mutation_id) DO NOTHING`) — never a per-mutation *replay of effects* | **idempotent** by `mutation_id` PK (`once()`-guarded, `07` :409); a re-run re-inserts nothing |
