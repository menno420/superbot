...
>
> 1. **Mining character platform** — **2026-06-15 handoff re-sets the stop-point after PRs #884 and #891 landed cleanly:** **PR #884** shipped the **§7.5 Vault v1** (additive per-player safe stash; atomic deposit/withdraw via `services/mining_workflow.py`; `!vault` / `!stash` / `!unstash`). **PR #891** shipped **Mining Slice D — capped skill tree (§7.4)** (`071_player_skills.sql`, `utils/mining/skills.py`, `services/skill_service.py` allocate/available/respec, `utils/equipment.py` `EffectiveStats` merge, `views/mining/skills_panel.py`, `!skills` / `!skill <branch>`; gates green / arch 0 / born-red flipped `complete` 2026-06-15). **Verified continuation set:** the canonical plan referenced by those PRs (`docs/planning/mining-structures-skill-tree-plan-2026-06-14.md`) was not present on disk in the mainline checkout; verify and re-commit or retire that reference before the next mining slice. **Next picks:** (A) Vault v2 soft-cap · (Slice E) respec service · (Slices B/F/G) Forge/Home. Details + discriminators: PR #893.
...
>
> **Last updated:** 2026-06-14, **seventh Q-0107 reconciliation pass (the band-#870 cadence
