...
- **#1054–#1061 (2026-06-18→19, consistency-linter lane + dashboard refreshes — ledger catch-up,
  recorded on sight per Q-0166)** — the consistency-linter slices are described in detail in the ▶
  Next-action callout above: **#1054** (Lane A1 — `views/selectors/` API-ripple set windowed,
  `select_option_truncation` 15→7), **#1056** (Lane A2 — per-panel embedded selects windowed, 7→0),
  **#1057** (rule 3 `panel_base_class` reconciled to the arch ratchet, 26→0), **#1058** (rule 1
  `edit_in_place` triaged, 45→17), **#1059** (rule 2 `back_button` triaged, 7→0), and **#1060**
  (self-initiated Q-0172 — promoted the AI-panel in-place-nav idea →
  [`planning/ai-panel-inplace-navigation-plan-2026-06-19.md`](planning/ai-panel-inplace-navigation-plan-2026-06-19.md),
  the blocker for graduating rule 1). **#1053** = the twelfth Q-0107 reconciliation pass (band-#1050,
  docs); **#1055** + **#1061** = `bot/dashboard-refresh` generated-data regens.
...
- **Older merges (#1028 … #535) → [`current-state-archive.md`](current-state-archive.md).** Recently-shipped keeps the ~20 newest; older entries are trimmed to the archive (newest-first), which `scripts/check_docs.py` soft-ratchets at 20 and `check_current_state_ledger.py` treats as present. *(The twelfth Q-0107 pass — band-#1050, 2026-06-18 — added the missing #1022 and #1029 entries and trimmed the live ledger back to the 20 newest, moving #1026 … #975 to the archive.)*
