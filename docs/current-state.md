...
>
> **▶ Next action — one live queue:** the **[band queue](planning/reconciliation-pass-2026-06-12-night.md)** (the second Q-0107 pass, 2026-06-12 night): **P2 doc-drift sweep** → **backup posture** → the **Q-0108–Q-0112 safety/community family plan + first slices** (now UX-referenced by [`ux/pattern-library.md`](ux/pattern-library.md) `pattern_id`s) → the carried **P0-3/P0-4/P0-2** hardening tracks. Owner-steered alternates + the deliberately-deferred list live in that doc §4; [`roadmap.md`](roadmap.md) stays the per-area index. **Status is per-lane below — a session edits ONLY its own lane's bullet** (this paragraph used to be one shared mega-line and collided on every parallel merge; convention: [`owner/ai-project-workflow.md`](owner/ai-project-workflow.md) §9 "Cross-cutting ledger discipline").
>
