...
>
> **▶ Next action:** Three active lanes (all PR #s reconciled 2026-06-09; verify open PRs live — **#620**, the test-only 062-storage integration test, was open at review time).
>
> 1. **Mining character platform** — Wave 1 continues. Shipped: foundation **#606**, "The Descent" **#607**, combat gear→deathmatch **#608**, sell/buy market **#609**, Character overview **#610**. **Next slices:** the audited **Workshop + durability** (the keystone recurring ore/coin sink, brainstorm §7.5) and the **mother-panel live overview** (§6.3 — the hub embed is still static). Route in: `docs/ideas/mining_exploration_brainstorm.md` §7.7 + the games folio.
> 2. **Adaptive Setup/Access platform** — Phase 0 complete; Phase 1 underway: Q-0026 identity repair + Phase 0 contracts **#588**, P1A Access Map projection **#589**, P0C groundwork **#591**, P0C seam conversion + P1B `routing_access_conflict` **#592**. **Next:** P1B's `help_advertises_locked` and P1C (read-only Access Map + Help Preview panels) are **blocked on the audience-simulation decision — now formalized as Q-0045**; the denial-message UX is Q-0036 (confirm with maintainer). Q-0028–Q-0033 hold at safe defaults; the server-management PR13 AI generation layer stays gated. Route in: plan §16.8.
> 3. **AI tooling (orchestration + answerability)** — orchestration Phases 1–3 shipped (**#612**, **#618**, **#619** — including the gate-lifted `ai:tools` Tools & Workflows operator UI; default byte-identical); answerability Phase 1A/1B (**#612**, Q-0043: range cash **inclusive**) + Phase 2 read model (**#616**) shipped. **Next:** orchestration **Phase 4** (complex-BTD6 plan/execute/verify workflow + typed answer contracts + the deferred §12.1 durable audit trace — **confirm the MVP slice first**) and/or answerability **Phase 3** (the scope-filtered self-awareness tools exposing the read model — net-new AI **exposure**: needs the gate explicitly lifted **and** a confirmed tool list). Plans: [`ai/ai-complex-request-tool-orchestration-plan.md`](ai/ai-complex-request-tool-orchestration-plan.md) · [`planning/ai-btd6-answerability-roadmap-2026-06-09.md`](planning/ai-btd6-answerability-roadmap-2026-06-09.md).
>
> Cross-cutting: **Community Spotlight** (side-lane **#613**/**#614** + hotfixes **#615**/**#617**) is live but **unregistered** in the subsystem/hub registries (Help-invisible) — integration depth + the greedy `!hub`/`!server` aliases are owner question **Q-0044**; the cog was hardened in the 2026-06-09 review session (canonical `utils/db/xp.py` read, `member_count` crash fix, first tests). Full repo review: [`audits/repo-review-2026-06-09.md`](audits/repo-review-2026-06-09.md).
>
> **Last updated:** 2026-06-09 (repo-review session) · Reconciled every stale "(this session)" / "reconcile next session" marker against live GitHub: orchestration Phase 3 = **#619**, Character overview = **#610**, sell/buy market = **#609**, Context Compiler = **#594**, P0C seam conversion = **#592**. Recorded the side-lane **Community Spotlight** lane (**#613**/**#614** + hotfixes **#615**/**#617**) and hardened it (canonical `utils/db/xp.py` read, `member_count=None` crash fix, first tests). Docs refreshed in the same pass: `docs/roadmap.md` horizons (AI + games are now active lanes; at-a-glance table), `docs/ownership.md` (role-threshold cell → audited seam), `docs/repo-navigation-map.md` (+`community_spotlight` row), `docs/help-command-surface-map.md` (§3 open-gap banner), both AI plans + the wire-exploration plan (PR #s), adaptive plan §15/§16.8. New owner questions: **Q-0044** (spotlight integration + aliases), **Q-0045** (audience simulation — formalizes plan §16.8 item 3). Repo health at review: CI mirror green (**8352 passed, 16 skipped**), architecture **0 errors** (87 known warnings), docs checks pass. Full findings: [`audits/repo-review-2026-06-09.md`](audits/repo-review-2026-06-09.md). Source and live GitHub state supersede older wording; verify open PRs live.
...
- **`new_subsystem.py` scaffold script (Q-0025).** Adding a hub/subsystem requires
  ...
  `docs/ideas/` as a decided backlog item. **Owner decision Q-0025.** (Community
  Spotlight registration — Q-0044 — is the first natural consumer once built.)
