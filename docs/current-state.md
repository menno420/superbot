...
>
> **▶ NEXT (live — read THIS line): the band-#900 queue is essentially drained — the next reconciliation pass (#930) re-plans.** **Security tiers 1+2 shipped (#929, `needs-hermes-review` — awaiting human merge)**; **BUG-0009 slices 1/2/2b all shipped** (#924 · #926). The remaining band-#900 slots are all **blocked**: P1-1 absence-guard Layer B + the live-quality battery (creds/review), BUG-0009 slice 3 newest-towers (**data-gated** — `towers.json` has no release-order field), and the owner-steered product/loop threads (mining V-16 phase-2 PNG pack, substrate-kit). So an autonomous run nearing #930 should let the reconciliation pass plan the next band rather than force a blocked slot. *The live next-step is THIS sentence — **not** any other "next ▶ =" lower in this callout or in the dated `Last updated:` stamps further down (those are historical reconciliation snapshots: e.g. a stamp still reads "next ▶ = mining Forge", but Forge shipped #905). Trust this line + the Recently-shipped list, never a stamp.*
>
...
- **#929 (2026-06-15, security tiers 1+2 — raid detection + account-age filter, Q-0111)** —
  band-#900 decade-queue slot 9; the safety/community lane's final approved slice. A new hub-less
  `security` subsystem mirroring welcome/automod: **off by default · fail-open · self-contained (no
  external API, no PII)**. **Tier 1 (raid detection):** a pure per-guild `RaidTracker` sliding
  window → on `>= N` joins / `T`s, a deduped staff alert (once per lockdown) + optional auto-slowmode
  on a configured channel (raised for a bounded window, then auto-restored). **Tier 2 (account-age
  filter):** an account younger than `N` days on join is `alert`-ed or `kick`-ed (kick routes through
  `moderation_service.kick` — no parallel action/audit path). Tiers 3+4 (alt-detection / VPN — DECLINED,
  GDPR) kept absent. Files: `services/security_service.py` + `security_config.py` (guardrail-clamped
  thresholds) + `cogs/security/schemas.py` (11 SettingSpecs) + `cogs/security_cog.py` (`on_member_join`
  listener + `!security` summary + Help hook) + the 6 new-subsystem cascade touch-points + the
  `security.raid_detected`/`security.account_flagged` advisory events. `mock_security_alerts` is the UX
  target. **Labelled `needs-hermes-review` (Q-0117) — NOT self-merged.** `check_quality --full` green
  (9920); arch 0; +37 tests. *(Process note: this session's shell cwd got stuck in `disbot/` after a
  `cd`, which deadlocked the repo's relative-path PreToolUse hooks; the feature code (already on disk)
  was preserved via a worktree-isolated agent commit, then finalized once cwd reset — see the session
  log's ⚠ note + the Q-0089 idea.)*
