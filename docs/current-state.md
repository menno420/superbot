...
>
> **▶ Next action — one live queue:** **EIGHTEENTH Q-0107 PASS DONE (2026-06-21, band-#1230, issue #1232 — [pass record + next-band queue](planning/reconciliation-pass-2026-06-21-band1230.md)).** This band's headline is the **reaction-roles / role-menu overhaul** (Carl-bot parity) shipped end-to-end — PR 1 audited `reaction_role_service` seam + `role_menus` data layer + migration 078 (#1220), PR 2 in-Discord role-menu builder (#1219), PR 3–5 together (#1227 — per-message emoji modes [migration 079] · free temp-roles `RoleGrantsCog`/`!temprole` [migration 080] · role-pickup analytics [migration 081]); only **PR 6 (PIL banner cards) + the gated web builder** remain ([plan](planning/reaction-roles-overhaul-plan-2026-06-21.md)). Also shipped: the **creature game** moved design→runtime — **catch + collection/dex** (#1208, `creature_cog.py`) and the **level-normalized PvP battle engine** (pure domain `utils/creatures/battle.py`, #1213), with the **user-facing PvP flow now shipping** (#1230, auto-merge armed by owner); the **"free for everyone, forever" product North Star** codified (Q-0190/#1226 · open-source/self-host posture #1228 · MIT-for-now license decision #1229/#1231); the **`public-data-contract-field-snapshot` redaction guard** (#1210 — `site.json` now fails closed on within-family leaf leaks, completing the ungated stdlib-guard cluster); workflow tooling (**permission-overlap guard** #1212, **lane-overlap claim-scan** reading `active-work.md` #1223, **Q-0189 open-the-PR-fast** #1224); and bug fixes **BUG-0020/0021/0022** (#1206) + **BUG-0023** botsite command-count reconcile + tool-pin drift guard (#1207). **No PLAN-BACKLOG-THIN flag** — buildable depth is well over the cadence (creature-PvP flow in flight + leaderboards · botsite-React migration · consistency-linter AI-nav PR 1 · procedures→skills Batch 2 · Pokétwo/MusicBot mapped features); **honest caveat (carried, unchanged):** the *cleanly-ungated self-merge* subset stays thin — most depth is runtime / `needs-hermes-review` — so the next dispatch should prefer a substantial review-gated lane or promote a fresh idea → plan → build (Q-0172). **▶ Next ungated startable = ONE of:** (a) **creature-game PvP is now buildable-complete for v1** — the user-facing flow shipped (#1230) **and** its result-recording + win/loss records + battle leaderboard shipped (this session: migration 082 `creature_battle_record` · `creature_battle_service.resolve_and_record_pvp` · `!cbattletop`/`!cbrecord` · `battle_win` xp), so lane (a)'s remaining slices are **later/gated** (Explore-hub Lane-B battle panel · balance/art Q-0187) — [plan](planning/creature-game-design-and-sim-2026-06-20.md) §4 — prefer (b)/(c)/(d) below for the next ungated build; (b) the **botsite React-SPA migration** ([plan](planning/botsite-react-spa-migration-plan-2026-06-20.md)); (c) a substantial **`needs-hermes-review`** lane — consistency-linter **AI-nav PR 1** ([plan](planning/ai-panel-inplace-navigation-plan-2026-06-19.md)) or **procedures→skills Batch 2** ([plan](planning/procedures-to-skills-conversion-plan-2026-06-17.md)); or (d) promote a fresh `docs/ideas/` idea → plan → build (Q-0172). Owner-paced/gated (not in the buildable count): reaction-roles PR 6 + web builder · creature PvP balance + art (Q-0187) · website rollout · feedback-board PR 1 (owner auth) · AI-ticket build (Q-0183) · Explore-hub PR 2 + gated layers (Q-0182) · dashboard writes/control-API (security review). *(Aggressive prune this pass per the standing Q-0102 finding — passes 10–17 were inlined here as a 38 KB wall; their per-band detail lives in each `planning/reconciliation-pass-*` record. Trust THIS line + the Recently-shipped list, never a lower "next ▶".)*
>
...
>
> **Last updated:** 2026-06-21 — **eighteenth Q-0107 reconciliation pass (band-#1230, issue #1232
> — [pass record](planning/reconciliation-pass-2026-06-21-band1230.md));** the live state is the
> **▶ Next action** line above + the Recently-shipped list. *(The dated narrative below is historical —
> passes 10–18 record their state in the ▶ Next action callout + their per-band pass records, not here.)*
...
>
> **Last reconciliation pass:** PR #1231 (2026-06-21, eighteenth Q-0107 cadence pass, band-#1230 —
> [the pass record + next-band queue](planning/reconciliation-pass-2026-06-21-band1230.md)). The next
> **docs-only review + planning reconciliation** is due once merged PRs cross #1260 (every
...
- **Older merges (#1170 … #535) → [`current-state-archive.md`](current-state-archive.md).** Recently-shipped keeps the ~20 newest; older entries are trimmed to the archive (newest-first), which `scripts/check_docs.py` soft-ratchets at 20 and `check_current_state_ledger.py` treats as present. *(Thematic grouping by date means the live/archive PR-number spans overlap slightly — the floor pointer is approximate prose, not a strict bound; the per-band pass records carry the exact moved sets.)* *(The eighteenth Q-0107 pass — band-#1230, 2026-06-21 — added the band #1203–#1231 work as seven grouped entries (reaction-roles overhaul #1215/#1216/#1217/#1218/#1220/#1219/#1227, creature design→runtime #1208/#1213, "free for everyone" North Star #1226/#1228/#1229/#1231, workflow tooling #1211/#1212/#1223/#1224/#1225, redaction guard #1210, bug fixes + CI #1203/#1205/#1206/#1207, dashboard-refresh #1209/#1214/#1222); trimmed the live ledger to 20, moving #1169 · #1145-band · #1142 · #1135-band · #1131 · #1132 · #1130 to the archive.)* *(The seventeenth Q-0107 pass — band-#1200, 2026-06-20 — added the band #1172–#1201 work as eight grouped entries; trimmed the live ledger to 20, moving #1129 · #1126 · #1125 · #1124 · #1115 · #1109 · #1112-band · #1101-band to the archive.)*
