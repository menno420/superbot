...
>
> **▶ Next action — one live queue:** the **[band queue](planning/reconciliation-pass-2026-06-12-night.md)** (the second Q-0107 pass, 2026-06-12 night): ~~P2 doc-drift sweep~~ (✅ #764) → ~~backup posture~~ (✅ #769) → the **Q-0108–Q-0112 safety/community family plan + first slices** (UX-referenced by [`ux/pattern-library.md`](ux/pattern-library.md) `pattern_id`s) → the carried **P0-3/P0-4/P0-2** hardening tracks. Owner-steered alternates + the deliberately-deferred list live in that doc §4; [`roadmap.md`](roadmap.md) stays the per-area index. **Status is per-lane below — a session edits ONLY its own lane's bullet** (this paragraph used to be one shared mega-line and collided on every parallel merge; convention: [`owner/ai-project-workflow.md`](owner/ai-project-workflow.md) §9 "Cross-cutting ledger discipline").
>
...
- **#769 (2026-06-13, Postgres backup posture — band slot 3)** — daily `pg_dump --no-owner --no-acl` against `DATABASE_PUBLIC_URL` (Railway external proxy) → `.sql.gz` → GitHub Actions artifact with 90-day retention; opens a bug issue on failure. Local ad-hoc backup: `scripts/backup_db.py`. Restore procedure + one-time owner setup (add `DATABASE_PUBLIC_URL` as a GitHub Actions secret) documented in [`docs/operations/production-deployment.md`](operations/production-deployment.md). Session idea (Q-0089): dump integrity check ([idea](ideas/backup-integrity-check-2026-06-13.md)). **Owner action required:** add `DATABASE_PUBLIC_URL` to GitHub Actions secrets before the 02:00 UTC scheduled run.
- **#767 (2026-06-13, executor-nightly cron reliability fix)** — shifted both nightly cron ticks from `0 1,3 * * *` → `17 1,3 * * *` (01:17 / 03:17 UTC) to avoid GitHub's `:00` scheduler congestion; a first-occurrence miss at 01:00 UTC prompted the investigation (first-registered cron + most-congested minute of the hour). Workflow behaviour unchanged.
- **#765 (2026-06-13, session close — autonomous loop live + Hermes dual-platform control plane)** — end-of-session docs: session log `2026-06-13-autonomous-loop-hermes-control-plane.md` (fleet state, 3 unconfirmed maintainer steps, the false-green-checker finding), idea file for Hermes bug-triage flow redesign, **BUG-0011** (Hermes gateway 409 crash-loop). Key finding: ledger was false-green mid-session (#763 fixed the regex root cause the same night).
