...
>
> **▶ Next action:** Phase 0 of the [Adaptive Setup, Access, Profile, and Routine Platform](planning/adaptive-setup-access-routine-platform-2026-06-08.md) is underway. The **Q-0026 multi-word subsystem identity repair is implemented** — `cog_name_to_subsystem` now converts CamelCase → snake_case, the registry key is `server_management`, and the same fix repaired the latent `proof_channel` / `four_twenty` collapse — and the Phase 0 **direct-vs-draft mutation boundary** (`docs/ownership.md` § "Direct vs. draft mutation lanes") and **access read-model contract** (planning doc §4 + §16) are now documented. **P1A is implemented** — `services/access_projection.py` is the side-effect-free composed Access Map read model (command-access + routing + governance + help axes, reusing the existing owners; reason schema reuses `command_access.DecisionReason`) with 19 tests; no UI, no persistence. **P0C groundwork is laid:** the role-threshold direct-write drift now has a shrinking drift-fence invariant (`tests/unit/invariants/test_no_direct_role_threshold_writes.py`) pinning all 6 sites, plus a turn-key swap recipe in planning §16.5. The next lanes are **P0C** (convert the 6 role-threshold sites onto the audited `role_automation` seam + shrink the fence), **P1B** (the three *runtime* drift providers in `setup_diagnostics` — not `identity_mismatch`, already covered — + locked-reason denial integration), and **P1C** (read-only Access Map + Help Preview panels; resolve the audience-simulation point in planning §16.8 first). Owner decisions Q-0017–Q-0027 merged via **#585**; Q-0028–Q-0033 hold at their safe defaults. The server-management PR13 AI generation layer remains gated by the AI-expansion gate.
>
> **Last updated:** 2026-06-08 · Adaptive Setup/Access platform Phase 0→1: **#588 merged** the **Q-0026 identity repair** + Phase 0 contracts; **#589 merged** **P1A** (the **Access Map projection service** `services/access_projection.py`, 19 tests); then P0C groundwork landed — a shrinking **role-threshold drift-fence invariant** + a turn-key swap recipe (planning §16.5) — plus §16.8 refinements for the next agent. Earlier the same day **#584** merged the Server Management Hub and **#585** merged Q-0017–Q-0027. Source and live GitHub state supersede older wording. This file lists merged work + the next action; verify open PRs live.
