...
- *(this session)* — **AI + BTD6 answerability — Phase 1A (deterministic round-cash query)**: implemented `btd6_data_service.round_cash(round_start, round_end=None)`, the BTD6-owned, read-only round / **inclusive**-range cash query — the deterministic owner derives the range total (`range_cash`) instead of asking the model to subtract cumulative endpoints. Returns structured per-round / range / cumulative-endpoint / starting-cash / `assumptions` fields, with structured `invalid_range` / `cash_unavailable` refusals (never a fabricated number). 20 new tests pin every row of the roadmap's Phase 1A semantics table; full CI mirror green (8263 passed). **No AI tool / context / UI change** — that is Phase 1B, still gated on the orchestration foundation (AR-10). Plan: [`planning/ai-btd6-answerability-roadmap-2026-06-09.md`](planning/ai-btd6-answerability-roadmap-2026-06-09.md). Reconcile PR # next session.
