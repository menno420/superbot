...
- **#922 (2026-06-15, welcome phase 2 — opt-in PIL welcome card)** — the next ▶ startable plan
  slice (band-#900 decade-queue slot 7, `ready` quick-win; Q-0110). Promotes the existing
  `render_welcome_card` UX-Lab prototype to a real, **opt-in** feature: a new `welcome_card_enabled`
  scalar setting (default OFF) makes the join greeting attach a rendered welcome banner. Production
  renderer `utils/welcome_render.py` (pure, lazy-PIL, `bytes | None`, no-network initials-disc
  avatar per the platform-limits §4 fallback; member top-role colour drives the accent);
  `WelcomePolicy.card_enabled` + a `renders_card` predicate folding it onto `greet_on_join` (the
  card is an *attachment to* the greeting, never standalone); `welcome_service.handle_member_join`
  renders off-thread + attaches fail-safe (Pillow-absent / render-fault → embed-only, greeting still
  posts); the `!welcome` summary shows the card flag. **Fully additive** — default OFF →
  byte-identical on every existing guild. Satisfies the P1-3 settings declared⇔consumed parity
  invariant (#917/#918) in both directions. `check_quality --full` green (9859); arch 0.
