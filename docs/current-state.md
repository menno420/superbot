...
>
> **▶▶ OWNER DIRECTIVE — for the next reconciliation pass (the #1140 fire, 2026-06-19): run it EXTRA-THOROUGH, weighted toward PLANNING, not just a ledger reconcile.** The goal this pass: **(1) surface the open QUESTIONS** from today's design captures into the question router (DISCUSS lane) — the AI correction-report/ticket service's *audience-routing + fail-closed redaction* design (`ideas/ai-correction-report-and-ticket-service-2026-06-19.md`), the federated-world XP/gear refinements + the Explore hub's literal Discord form (`ideas/explore-hub-federated-world-2026-06-19.md`), memory global-vs-per-guild (`ideas/honcho-memory-evaluation`), and the public bot-site's one-line pitch — **and (2) promote today's idea backlog into buildable `docs/planning/` plans + `roadmap.md` horizons.** Priority: the **federated Explore hub** spine first (it homes fishing/mining/pets/survival), then the feedback-board/owner-inbox generalization; the **AI-ticket service stays plan-the-questions-first** (owner flagged it needs its own session — don't write its build plan yet, just route its questions). The idea→build gate is open (Q-0172), so **stock a FULL buildable band** so tonight's routines have execute-ready plans. Consume + clear this directive when the #1140 pass runs. *(Unrelated to the routine: a condensed owner-facing voice-brainstorm pack now lives at [`operations/voice-brainstorm-pack.md`](operations/voice-brainstorm-pack.md).)*
>
...
>
> **Last reconciliation pass:** PR #1110 (2026-06-19, fourteenth Q-0107 cadence pass —
> [the pass record + next-band queue](planning/reconciliation-pass-2026-06-19-band1110.md)). The next
> **docs-only review + planning reconciliation** is due once merged PRs cross #1140 (every
...
- **#1099 · #1100 · #1102 · #1104 · #1107 · #1110 (2026-06-19, website two-site split — planning band, Q-0178/Q-0179)** —
  ...
  control-panel-placement decision Q-0179 (#1102), **locking the open decisions + resolving Q-0179** (#1104),
  making the §5 decomposition truly file-disjoint pre-ultracode (#1107), and the owner site-identity vision +
  fan-out enablers (#1110). Owner decisions LOCKED 2026-06-19 (plan §7) — the build run is decision-unblocked.
