...
MCP startup: pinned in **`.mcp.json`** (`npx -y @optave/codegraph@3.11.2 mcp`); the SessionStart hook builds the index with the same pin (**`scripts/claude_session_start.sh`** `CG_PKG`). Both were bumped 3.10.0 → 3.11.2 on 2026-06-08 and verified in build + CLI + MCP-server `initialize`. **Honest verdict: 3.11.2 produces a _measurably identical_ graph on this repo** — same 31564 nodes, same `extends` (193) / `receiver` (67) counts, same ~20% caller coverage, same `dead-unresolved` rate; 3.11.x's fixes (watch-rebuild `calls`-edge over-counting, restored receiver edges) target regressions this repo doesn't exhibit. Adopted for **currency + no downside**, not new capability. The CodeGraph reliability tiers below were **re-verified byte-identical** (bare-token / `dead-unresolved`), so they still hold. New-version provenance (owner decision Q-0014): **confirmed working in the live harness 2026-06-08** — a SessionStart banner reported `v3.11.2` with a clean full rebuild (31735 nodes) and `mcp__codegraph__where` resolves freshly-written symbols (`plan_template` in the new `setup_role_templates.py`); the documented `dead-unresolved` / qualified-call (`module.fn()`) misses still hold, so the trust tiers are intact. If it ever regresses, revert the two repo pins. **Availability gotcha:** the SessionStart hook can report `[CodeGraph] CLI unavailable` on a *cold* container when the first `npx -y` download blips (the npx cache `/root/.npm/_npx` starts empty) — this silently disabled CodeGraph for a whole session on 2026-06-08. The hook now **retries the probe 3× and prints the real npm error** (`Last error:` line) so a genuine bad-version/registry failure is diagnosable instead of a vague skip (`scripts/claude_session_start.sh`, 2026-06-08). If a session boots with CodeGraph "unavailable", read that `Last error:` line and re-run `npx -y @optave/codegraph@3.11.2 build .` — a transient blip clears on retry.
